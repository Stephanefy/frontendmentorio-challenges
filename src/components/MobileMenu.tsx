...
const MobileMenu: FC<Props> = (props): JSX.Element => {
  ...
  return (
    <div className="absolute top-64 left-1/2 w-[234px] -translate-x-1/2 -translate-y-1/2 transform rounded-lg bg-white pr-3">
      <ul className="w-full pl-3 py-3">
        <h3 className="pl-3 font-bold uppercase text-xs text-primary-gray">
          <span className="mr-2">all</span><span>boards</span> <span className="ml-1">({boards.length})</span>
        </h3>
        {boards.map((board, index) => (
          <li key={index} className={`pl-3 relative py-3 group text-primary-gray ${state.name === boards[index].name ? "hover:text-white" : "hover:text-primary"}`}>
            ...
            >
              <svg 
                ...
                className={` 
                ${
                  state.name === boards[index].name ? "fill-white" : "fill-primary-gray group-hover:fill-primary"
                }
                `}
              >
                <path
        ...
        <li className="pl-3 relative py-3 text-primary-gray group">
