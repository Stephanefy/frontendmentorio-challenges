...
function Header({currentJob} : any) {
  ...
  return (
    <div 
    className='flex flex-col md:flex-row items-center justify-center md:justify-between absolute left-1/2 -translate-x-1/2 top-10 mt-24 h-52 w-5/6 lg:w-[700px] md:h-[140px] bg-white rounded-md' >
        <div className="w-10 h-10 md:w-36 md:h-36 flex justify-center items-center absolute md:static -top-4 p-2 rounded-md" style={{backgroundColor: logoBackground}}>
            <img src={`/src/${logo}`} width={81} />
        </div>
        <div className="w-full md:w-6/12 md:pl-6 flex flex-col justify-center items-center md:items-start">
            <h3 className="h-10">{company}</h3>
