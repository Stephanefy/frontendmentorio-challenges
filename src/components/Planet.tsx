    ...
    return (
        ...
        <article className="w-full text-white h-full md:max-w-[1440px] mx-auto overflow-x-hidden">
            <ul className="mobile-tab md:hidden flex my-3 justify-around w-full">
                    <li
                    style={{
                        borderBottom: currentOpenedTab === 0 ? `4px solid ${currentPlanet?.color}` : 'none'
                    }}
                    ><button className={`mobile-tab-item w-full h-[48] uppercase my-2 py-3 flex justify-between font-['league_Spartan'] font-bold tracking-wider text-[12px]`}   onClick={() => setCurrentOpenedTab(0)}><span className='flex-1 text-left'>Overview</span></button></li>
                    <li
                        style={{
                        borderBottom: currentOpenedTab === 1 ? `4px solid ${currentPlanet?.color}` : 'none'
                    }}
                    ><button className={`w-full h-[48] uppercase my-2  py-3 flex justify-between font-['league_Spartan'] font-bold tracking-wider text-[12px]`}  onClick={() => setCurrentOpenedTab(1)}><span className='flex-1 text-left'>Structure</span></button></li>
                    <li
                        style={{
                        borderBottom: currentOpenedTab === 2 ? `4px solid ${currentPlanet?.color}` : 'none'
                    }}
                    ><button className={`w-full h-[48] uppercase my-2  py-3 flex justify-between font-['league_Spartan'] font-bold tracking-wider text-[12px]`}  onClick={() => setCurrentOpenedTab(2)}><span className='flex-1 text-left'>Surface</span></button></li>
            </ul>
                ...
                <div className="basis-6/6 lg:basis-4/6 flex lg:flex-col items-center justify-center lg:items-start">
                    <div className='w-12/12 text-center md:text-left md:w-7/12 '>
                        ...
                        <div className='flex items-center gap-2'>
                            <a href={`
                                ${currentOpenedTab === 0 ? currentPlanet?.overview.source : currentOpenedTab === 1 ? currentPlanet?.structure.source : currentPlanet?.geology.source}
                            `} className="my-8"><span className="text-custom_grey ">Source: </span>
                                Wikipedia
                            </a>
            ...
            <section className="max-w-3xl lg:max-w-none lg:w-full flex flex-col md:flex-row justify-between lg:mb-24 mt-32 overflow-x-hidden px-3 mx-auto">
                <article className="planet--features flex justify-between items-center md:block border-2 border-white my-1 md:my-3 mx-auto md:mx-3 p-3 lg:p-6 text-left w-full md:w-72">
                    <h4 className="text--grey text-xs lg:text-lg">Rotation Time</h4>
                    <p className="text-2xl lg:text-4xl mt-2 uppercase">{currentPlanet?.rotation}</p>
                </article>
                <article className="planet--features flex justify-between items-center md:block  border-2 border-white my-1 md:my-3 mx-auto md:mx-3 p-3 lg:p-6 text-left w-full md:w-72">
                    <h4 className="text--grey text-xs lg:text-lg">Revolution Time</h4>
                ...
                <article className="planet--features flex justify-between items-center md:block  border-2 border-white my-1 md:my-3 mx-auto md:mx-3 p-3 lg:p-6 text-left w-full md:w-72">
                    <h4 className="text--grey text-xs lg:text-lg">Average Temp</h4>
